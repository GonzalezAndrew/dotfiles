	 ...
	 powerbg=${pink}
      ...
      if $show_time; then
        if $show_day_month && $show_military ; then # military time and dd/mm
          tmux set-option -ga status-right "#[fg=${dark_purple},bg=${powerbg},nobold,nounderscore,noitalics] ${right_sep}#[fg=${white},bg=${dark_purple}] %A, %B %-e, %Y, %-l:%M:%S%p %Z%0z "
	...
	tmux set-option -ga status-right "#[fg=${dark_gray},bg=${pink}] #($current_dir/gpu_usage.sh) "
      ...
      if $show_time; then
        if $show_day_month && $show_military ; then # military time and dd/mm
          ...
          tmux set-option -ga status-right "#[fg=${white},bg=${dark_purple}] %A, %B %-e, %Y, %-l:%M:%S%p ${timezone} "
        fi
      fi
